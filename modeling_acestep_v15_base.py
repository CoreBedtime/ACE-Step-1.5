        ...
        with torch.no_grad():
            for step_idx, (t_curr, t_prev) in enumerate(iterator):
                ...
                if infer_method == "sde":
                    ...
                    # 2nd-order Heun method for higher quality
                    dt = t_curr - t_prev
                    dt_tensor = dt * torch.ones((bsz,), device=device, dtype=dtype).unsqueeze(-1).unsqueeze(-1)
                    
                    # 1. Prediction step (Euler)
                    xt_next = xt - vt * dt_tensor
                    
                    # 2. Correction step (requires another model pass)
                    x_next_batch = torch.cat([xt_next, xt_next], dim=0) if do_cfg_guidance else xt_next
                    ...
                    # Final Heun update
                    xt = xt - 0.5 * (vt + vt_next) * dt_tensor
