...
class AceStepHandler:
    ...
    def __init__(self):
        ...
        self.debug_stats = os.environ.get("ACESTEP_DEBUG_STATS", "").lower() in (
            "1",
            "true",
            "yes",
        )
        ...
        self.use_lora = False
        self.lora_scale = 1.0  # LoRA influence scale (0-2.0)
    ...
    @staticmethod
    def _normalize_lora_adapter(lora_path: str, tmp_dir: str) -> str:
        """Create a temp copy of the adapter dir with normalized safetensors keys.

        Collapses extra base_model.model. nesting produced by Fabric-wrapped
        saves so that PeftModel.from_pretrained can match them.

        Returns:
            Path to the (possibly temp) adapter directory to load from.
        """
        import shutil
        from safetensors.torch import load_file, save_file

        st_src = os.path.join(lora_path, "adapter_model.safetensors")
        if not os.path.exists(st_src):
            return lora_path  # nothing to fix, let PEFT handle .bin etc.

        state = load_file(st_src)
        needs_fix = any("base_model.model.base_model.model." in k for k in state)
        if not needs_fix:
            return lora_path  # keys are fine, use original

        # Copy whole adapter dir, then overwrite the safetensors with fixed keys
        shutil.copytree(lora_path, tmp_dir, dirs_exist_ok=True)
        normalized = {}
        for key, value in state.items():
            ...
            normalized[k] = value
        save_file(normalized, os.path.join(tmp_dir, "adapter_model.safetensors"))
        logger.info(f"Normalized {len(normalized)} LoRA keys into temp dir")
        return tmp_dir
    ...
    def load_lora(self, lora_path: str) -> str:
        ...
        try:
            ...
            # Disable TorchAO LoRA dispatch to avoid version-mismatch crashes
            # (TorchaoLoraLinear requires a kwarg not present in all torchao builds)
            _torchao_patch = None
            try:
                import peft.import_utils as _peft_iu

                if (
                    getattr(_peft_iu, "is_torchao_available", None)
                    and _peft_iu.is_torchao_available()
                ):
                    _torchao_patch = _peft_iu.is_torchao_available
                    _peft_iu.is_torchao_available = lambda: False
                    logger.info("Temporarily disabled PEFT TorchAO dispatch")
            except Exception:
                pass

            # Normalize keys (fixes Fabric triple-nesting) into a temp dir,
            # then let PeftModel.from_pretrained handle adapter-name mapping.
            try:
                with tempfile.TemporaryDirectory() as tmp_dir:
                    load_path = self._normalize_lora_adapter(lora_path, tmp_dir)
                ...
                if _torchao_patch is not None:
                    _peft_iu.is_torchao_available = _torchao_patch
    ...
    def set_lora_scale(self, scale: float) -> str:
        """Set LoRA adapter scale/weight (0-2.0 range).

        Args:
            scale: LoRA influence scale (0=disabled, 1=full effect, 2=double effect)
        ...
        try:
            ...
            return "❌ PEFT library not found. Cannot set LoRA scale."

        # Clamp scale to 0-2.0 range
        self.lora_scale = max(0.0, min(2.0, scale))

        # Only apply scaling if LoRA is enabled
        if not self.use_lora:
            logger.info(
                f"LoRA scale set to {self.lora_scale:.2f} (will apply when LoRA is enabled)"
            )
        ...
        try:
            ...
            for name, module in self.model.decoder.named_modules():
                # Only modify actual LoRA modules - check if it's a LoraLayer instance
                # This prevents modifying attention scaling and other non-LoRA modules
                if isinstance(module, LoraLayer) and hasattr(module, "scaling"):
            ...
            if modified_count > 0:
                logger.info(
                    f"LoRA scale set to {self.lora_scale:.2f} (modified {modified_count} modules)"
                )
        ...
        try:
            if device == "auto":
                ...
                if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
                    logger.warning(
                        "[initialize_service] CUDA requested but unavailable. Falling back to MPS."
                    )
                    device = "mps"
                elif hasattr(torch, "xpu") and torch.xpu.is_available():
                    logger.warning(
                        "[initialize_service] CUDA requested but unavailable. Falling back to XPU."
                    )
                    device = "xpu"
                else:
                    logger.warning(
                        "[initialize_service] CUDA requested but unavailable. Falling back to CPU."
                    )
                    device = "cpu"
                ...
                if torch.cuda.is_available():
                    logger.warning(
                        "[initialize_service] MPS requested but unavailable. Falling back to CUDA."
                    )
                    device = "cuda"
                elif hasattr(torch, "xpu") and torch.xpu.is_available():
                    logger.warning(
                        "[initialize_service] MPS requested but unavailable. Falling back to XPU."
                    )
                    device = "xpu"
                else:
                    logger.warning(
                        "[initialize_service] MPS requested but unavailable. Falling back to CPU."
                    )
                    device = "cpu"
                ...
                if torch.cuda.is_available():
                    logger.warning(
                        "[initialize_service] XPU requested but unavailable. Falling back to CUDA."
                    )
                    device = "cuda"
                elif (
                    hasattr(torch.backends, "mps") and torch.backends.mps.is_available()
                    ...
                    logger.warning(
                        "[initialize_service] XPU requested but unavailable. Falling back to MPS."
                    )
                    device = "mps"
                else:
                    logger.warning(
                        "[initialize_service] XPU requested but unavailable. Falling back to CPU."
                    )
                    device = "cpu"
            ...
            if os.path.exists(acestep_v15_checkpoint_path):
                ...
                for candidate in attn_candidates:
                    try:
                        logger.info(
                            f"[initialize_service] Attempting to load model with attention implementation: {candidate}"
                        )
                        ...
                        last_attn_error = e
                        logger.warning(
                            f"[initialize_service] Failed to load model with {candidate}: {e}"
                        )
                ...
                if os.path.exists(silence_latent_path):
                    self.silence_latent = torch.load(
                        silence_latent_path, weights_only=True
                    ).transpose(1, 2)
    ...
    def switch_to_training_preset(self) -> Tuple[str, bool]:
        ...
        if not self.last_init_params:
            return (
                "Cannot switch preset automatically: no previous init parameters found.",
                False,
            )
        ...
        if ok:
            return (
                f"Switched to training preset (quantization disabled).\n{status}",
                True,
            )
    ...
    def _estimate_diffusion_per_step(
        ...
        device_records = [
            r for r in records if r.get("device") == self.device
        ] or records
        ...
        per_steps = [
            r.get("per_step_sec") for r in device_records if r.get("per_step_sec")
        ]
        if per_steps:
            per_steps.sort()
    ...
    def _start_diffusion_progress_estimator(
        ...
        thread = threading.Thread(
            target=_runner, name="diffusion-progress", daemon=True
        )
        thread.start()
            ...
            if isinstance(refer_audio_list, list):
                for idx, refer_audio in enumerate(refer_audio_list):
                    refer_audio_list[idx] = (
                        refer_audio_list[idx].to(self.device).to(self._get_vae_dtype())
                    )
    ...
    def service_generate(
        ...
        if timesteps is not None:
            generate_kwargs["timesteps"] = torch.tensor(
                timesteps, dtype=torch.float32, device=self.device
            )
    ...
    def _tiled_decode_gpu(self, latents, B, T, stride, overlap, num_steps):
        ...
        for i in tqdm(
            range(num_steps), desc="Decoding audio chunks", disable=self.disable_tqdm
        ):
    ...
    def _tiled_decode_offload_cpu(self, latents, B, T, stride, overlap, num_steps):
        ...
        for i in tqdm(
            range(1, num_steps), desc="Decoding audio chunks", disable=self.disable_tqdm
        ):
            # Core range in latents
            core_start = i * stride
            core_end = min(core_start + stride, T)
    ...
    def _tiled_encode_gpu(self, audio, B, S, stride, overlap, num_steps, chunk_size):
        ...
        for i in tqdm(
            range(num_steps), desc="Encoding audio chunks", disable=self.disable_tqdm
        ):
    ...
    def _tiled_encode_offload_cpu(
        ...
        # Process remaining chunks
        for i in tqdm(
            range(1, num_steps), desc="Encoding audio chunks", disable=self.disable_tqdm
        ):
            # Core range in audio samples
            core_start = i * stride
            core_end = min(core_start + stride, S)
        ...
        try:
            ...
            with torch.inference_mode():
                with self._load_model_context("vae"):
                    ...
                    _vae_cpu = _os.environ.get("ACESTEP_VAE_ON_CPU", "0").lower() in (
                        "1",
                        "true",
                        "yes",
                    )
                    if _vae_cpu:
                        logger.info(
                            "[generate_music] Moving VAE to CPU for decode (ACESTEP_VAE_ON_CPU=1)..."
                        )
                    ...
                    if _vae_cpu:
                        logger.info(
                            "[generate_music] VAE decode on CPU complete, restoring to GPU..."
                        )
