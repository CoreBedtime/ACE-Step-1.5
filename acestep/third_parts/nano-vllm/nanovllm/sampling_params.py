...
class SamplingParams:
    ...
    cfg_scale: float = (
        1.0  # CFG guidance scale. When > 1.0, applies classifier-free guidance
    )
    top_k: Optional[int] = None  # Top-k sampling: consider only top k tokens
    top_p: Optional[float] = (
        None  # Top-p (nucleus) sampling: consider tokens with cumulative probability <= top_p
    )
    min_p: Optional[float] = (
        None  # Min-p sampling: filters out tokens with probability < min_p * max_prob
    )
    repetition_penalty: float = (
        1.0  # Repetition penalty: >1.0 reduces repetition, <1.0 increases it
    )
    ...
    logits_processor_update_state: Optional[Callable[[int], None]] = field(
