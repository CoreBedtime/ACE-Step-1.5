...
def apply_min_p(
    ...
    """Apply min-p mask to the logits.

    Min-p sampling filters out tokens with probability < min_p * max_prob.
    This helps maintain diversity while filtering out very unlikely tokens.
    ...
    # Get max probability for each sequence
    max_probs = probs.max(dim=-1, keepdim=True).values
    # Calculate threshold: min_p * max_prob
    threshold = min_p.unsqueeze(dim=1) * max_probs
...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        """
        Sample tokens from logits with optional top-k, top-p, and min-p filtering.
        ...
        # Apply min-p filtering first (before top-k/top-p)
