...
def find_available_port(start_port: int = 2333, max_attempts: int = 100) -> int:
    ...
    for i in range(max_attempts):
        ...
        try:
            ...
            continue
    raise RuntimeError(
        f"Could not find an available port starting from {start_port} after {max_attempts} attempts"
    )
...
class ModelRunner:
    def __init__(self, config: Config, rank: int, event: Event | list[Event]):
        ...
        config_dtype = getattr(
            hf_config, "dtype", getattr(hf_config, "torch_dtype", torch.bfloat16)
        )
        ...
        if config_dtype is None:
            ...
            not isinstance(config_dtype, torch.dtype)
            or not config_dtype.is_floating_point
        ):
    ...
    def _allocate_sample_buffers(self):
        ...
        self._cpu_temperatures = torch.zeros(
            max_bs, dtype=torch.float32, device="cpu", pin_memory=True
        )
        self._cpu_cfg_scales = torch.zeros(
            max_bs, dtype=torch.float32, device="cpu", pin_memory=True
        )
        self._cpu_top_ks = torch.zeros(
            max_bs, dtype=torch.int32, device="cpu", pin_memory=True
        )
        self._cpu_top_ps = torch.zeros(
            max_bs, dtype=torch.float32, device="cpu", pin_memory=True
        )
        self._cpu_min_ps = torch.zeros(
            max_bs, dtype=torch.float32, device="cpu", pin_memory=True
        )
        self._cpu_repetition_penalties = torch.zeros(
            max_bs, dtype=torch.float32, device="cpu", pin_memory=True
        )

        # Pre-allocate decode buffers on CPU with pinned memory
        self._cpu_input_ids = torch.zeros(
            max_bs, dtype=torch.int64, device="cpu", pin_memory=True
        )
        self._cpu_positions = torch.zeros(
            max_bs, dtype=torch.int64, device="cpu", pin_memory=True
        )
        self._cpu_slot_mapping = torch.zeros(
            max_bs, dtype=torch.int32, device="cpu", pin_memory=True
        )
        self._cpu_context_lens = torch.zeros(
            max_bs, dtype=torch.int32, device="cpu", pin_memory=True
        )

        # Pre-allocate prefill buffers on CPU with pinned memory (optimization to avoid repeated tensor creation)
        self._cpu_prefill_input_ids = torch.zeros(
            max_tokens, dtype=torch.int64, device="cpu", pin_memory=True
        )
        self._cpu_prefill_positions = torch.zeros(
            max_tokens, dtype=torch.int64, device="cpu", pin_memory=True
        )
        self._cpu_prefill_cu_seqlens = torch.zeros(
            max_bs + 1, dtype=torch.int32, device="cpu", pin_memory=True
        )
        self._cpu_prefill_slot_mapping = torch.zeros(
            max_tokens, dtype=torch.int32, device="cpu", pin_memory=True
        )

        # Pre-allocate block tables buffer (shared by both decode and prefill)
        self._cpu_block_tables = torch.zeros(
            max_bs, max_num_blocks, dtype=torch.int32, device="cpu", pin_memory=True
        )
        ...
        self._seq_token_ids_buffer = torch.zeros(
            max_bs,
    ...
    def prepare_sample(self, seqs: list[Sequence], is_cfg_batch: bool = False):
        ...
        for i, seq in enumerate(target_seqs):
            ...
            if seq.min_p is not None and seq.min_p > 0:
                min_ps_is_zero = False
            self._cpu_repetition_penalties[i] = (
        ...
        top_ks = (
            self._cpu_top_ks[:num_seqs].cuda(non_blocking=True)
            if not top_ks_is_zero
            else None
        )
        top_ps = (
            self._cpu_top_ps[:num_seqs].cuda(non_blocking=True)
            if not top_ps_is_one
            else None
        )
        min_ps = (
            self._cpu_min_ps[:num_seqs].cuda(non_blocking=True)
            if not min_ps_is_zero
            else None
        )
        repetition_penalties = (
            self._cpu_repetition_penalties[:num_seqs].cuda(non_blocking=True)
            if not repetition_penalties_is_one
            else None
        )
    ...
    def run_model(
        ...
        if is_prefill or self.enforce_eager or input_ids.size(0) > 512:
            _debug_log(
                f"run_model: eager mode, is_prefill={is_prefill}, bs={input_ids.size(0)}"
            )
            ...
            if context.block_tables.size(1) > max_num_blocks:
                ...
                _debug_log(
                    f"  fallback: block_tables cols {context.block_tables.size(1)} > max {max_num_blocks}"
                )
                out = self.model.compute_logits(self.model(input_ids, positions))
                debug_end("run_model", _t0, prefix="tensor.vllm")
                return out
            ...
            if context.block_tables.size(0) != bs:
                ...
                _debug_log(
                    f"  fallback: block_tables rows {context.block_tables.size(0)} != bs {bs}"
                )
                out = self.model.compute_logits(self.model(input_ids, positions))
                debug_end("run_model", _t0, prefix="tensor.vllm")
                return out
            ...
            if _DEBUG:
                ...
                min_block_id = (
                    context.block_tables[context.block_tables >= 0].min().item()
                    if (context.block_tables >= 0).any()
                    else -1
                )
                ...
                if max_block_id >= self.config.num_kvcache_blocks:
                    _debug_log(
                        f"  WARNING: block_table contains invalid block_id {max_block_id} >= {self.config.num_kvcache_blocks}"
                    )
    ...
    def run(self, seqs: list[Sequence], is_prefill: bool) -> list[int]:
        ...
        for i, seq in enumerate(seqs):
            _debug_log(
                f"  seq[{i}]: len={len(seq)}, num_blocks={seq.num_blocks}, "
        ...
        if is_cfg_batch:
            ...
            sample_params = (
                self.prepare_sample(seqs, is_cfg_batch=True) if self.rank == 0 else None
            )
            if sample_params is not None:
                ...
                temperatures = cfg_scales = top_ks = top_ps = min_ps = (
                    repetition_penalties
                ) = None
            ...
            if self.rank == 0:
                    ...
                    for i, seq in enumerate(cond_seqs):
                        ...
                        if penalty != 1.0:
                            ...
                            completion_tokens = torch.tensor(
                                seq.completion_token_ids, device=logits_cond.device
                            )
                            if len(completion_tokens) > 0:
                                ...
                                logits_cond[i] = torch.where(
                                    token_mask, penalty_scores, logits_cond[i]
                                )
                ...
                logits_cfg = logits_uncond + cfg_scales_tensor * (
                ...
                for i, seq in enumerate(cond_seqs):
                    if seq.logits_processor is not None:
                        ...
                        seq_input_ids = torch.tensor(
                            [seq.token_ids], device=logits_cfg.device
                        )
                        # Apply processor to this sequence's logits
                        logits_cfg[i : i + 1] = seq.logits_processor(
                            seq_input_ids, logits_cfg[i : i + 1]
                        )
            ...
            sample_params = (
                self.prepare_sample(seqs, is_cfg_batch=False)
                if self.rank == 0
                else None
            )
            if sample_params is not None:
            ...
            if self.rank == 0:
                ...
                if repetition_penalties is not None:
                    for i, seq in enumerate(seqs):
                        ...
                        if penalty != 1.0:
                            # Only penalize completion tokens (not prompt tokens)
                            completion_tokens = torch.tensor(
                                seq.completion_token_ids, device=logits.device
                            )
                            if len(completion_tokens) > 0:
                                # Create token mask: mark tokens that appeared in completion
                                token_mask = torch.zeros(
                                ...
                                logits[i] = torch.where(
                                    token_mask, penalty_scores, logits[i]
                                )
                ...
                for i, seq in enumerate(seqs):
                    if seq.logits_processor is not None:
                        # Create input_ids tensor for this sequence
                        seq_input_ids = torch.tensor(
                            [seq.token_ids], device=logits.device
                        )
                        # Apply processor to this sequence's logits (clone to avoid inference mode issues)
                        processed = seq.logits_processor(
                            seq_input_ids, logits[i : i + 1].clone()
                        )
