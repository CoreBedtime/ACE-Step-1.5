...
def _warn_if_prerelease_python():
    v = sys.version_info
    if getattr(v, "releaselevel", "final") != "final" and sys.platform.startswith(
        "linux"
    ):
        warnings.warn(
...
class LLMHandler:
    ...
    def __init__(self, persistent_storage_path: Optional[str] = None):
        ...
        self.disable_tqdm = os.environ.get("ACESTEP_DISABLE_TQDM", "").lower() in (
            "1",
            "true",
            "yes",
    ...
    def get_gpu_memory_utilization(
        ...
        try:
            ...
            if available_gpu >= minimal_gpu:
                ...
                ratio = min(
                    max_ratio, max(min_ratio, (available_gpu * 0.8) / total_gpu)
                )
    ...
    def _setup_constrained_processor(
        ...
        use_phase_temperatures = not is_batch and (
            metadata_temperature is not None or codes_temperature is not None
        )
    ...
    def _load_pytorch_model(self, model_path: str, device: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            logger.info(
                f"5Hz LM initialized successfully using PyTorch backend on {device}"
            )
            ...
            return (
                False,
                f"❌ Error initializing 5Hz LM: {str(e)}\n\nTraceback:\n{traceback.format_exc()}",
            )

    def _apply_top_k_filter(
        self, logits: torch.Tensor, top_k: Optional[int]
    ) -> torch.Tensor:
    ...
    def _check_eos_token(
        self, tokens: torch.Tensor, eos_token_id: int, pad_token_id: Optional[int]
    ) -> bool:
    ...
    def _normalize_batch_input(
        self, formatted_prompts: Union[str, List[str]]
    ) -> Tuple[List[str], bool]:
        ...
        try:
            if device == "auto":
                ...
                if hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
                    logger.warning(
                        "[initialize] CUDA requested but unavailable. Falling back to MPS."
                    )
                    device = "mps"
                elif hasattr(torch, "xpu") and torch.xpu.is_available():
                    logger.warning(
                        "[initialize] CUDA requested but unavailable. Falling back to XPU."
                    )
                    device = "xpu"
                else:
                    logger.warning(
                        "[initialize] CUDA requested but unavailable. Falling back to CPU."
                    )
                    device = "cpu"
                ...
                if torch.cuda.is_available():
                    logger.warning(
                        "[initialize] MPS requested but unavailable. Falling back to CUDA."
                    )
                    device = "cuda"
                elif hasattr(torch, "xpu") and torch.xpu.is_available():
                    logger.warning(
                        "[initialize] MPS requested but unavailable. Falling back to XPU."
                    )
                    device = "xpu"
                else:
                    logger.warning(
                        "[initialize] MPS requested but unavailable. Falling back to CPU."
                    )
                    device = "cpu"
                ...
                if torch.cuda.is_available():
                    logger.warning(
                        "[initialize] XPU requested but unavailable. Falling back to CUDA."
                    )
                    device = "cuda"
                elif (
                    hasattr(torch.backends, "mps") and torch.backends.mps.is_available()
                    ...
                    logger.warning(
                        "[initialize] XPU requested but unavailable. Falling back to MPS."
                    )
                    device = "mps"
                else:
                    logger.warning(
                        "[initialize] XPU requested but unavailable. Falling back to CPU."
                    )
                    device = "cpu"
            ...
            if lm_model_path is None:
                ...
                logger.info(
                    f"[initialize] lm_model_path is None, using default: {lm_model_path}"
                )
            ...
            llm_tokenizer = AutoTokenizer.from_pretrained(
                full_lm_model_path, use_fast=True
            )
            logger.info(
                f"5Hz LM tokenizer loaded successfully in {time.time() - start_time:.2f} seconds"
            )
            ...
            logger.info(
                f"Constrained processor initialized in {time.time() - processor_start:.2f} seconds"
            )
            ...
            if backend == "mlx" or (backend == "vllm" and device == "mps"):
                ...
                if self._is_mlx_available():
                    logger.info(
                        "Attempting MLX backend for Apple Silicon acceleration..."
                    )
                    ...
                    if mlx_success:
                        ...
                        if backend == "mlx":
                            ...
                            logger.warning(
                                "MLX explicitly requested but failed, falling back to PyTorch backend"
                            )
                            success, status_msg = self._load_pytorch_model(
                                full_lm_model_path, device
                            )
                            if not success:
                    ...
                    logger.warning(
                        "MLX not available (requires Apple Silicon + mlx-lm package)"
                    )
                ...
                if status_msg.startswith("❌"):
                    ...
                    if not self.llm_initialized:
                        ...
                        if device == "mps" and self._is_mlx_available():
                            ...
                            mlx_success, mlx_status = self._load_mlx_model(
                                full_lm_model_path
                            )
                            if mlx_success:
                                return mlx_status, True
                            logger.warning(
                                f"MLX also failed: {mlx_status}, falling back to PyTorch"
                            )
    ...
    def _initialize_5hz_lm_vllm(
        self, model_path: str, enforce_eager: bool = False
    ) -> str:
        ...
        try:
            ...
            gpu_memory_utilization, low_gpu_memory_mode = (
                self.get_gpu_memory_utilization(
    ...
    def _run_vllm(
        ...
        sampling_params = SamplingParams(
            ...
            logits_processor_update_state=constrained_processor.update_state
    ...
    def _run_pt_single(
        ...
        with self._load_model_context():
            ...
            if cfg_scale > 1.0:
                ...
                batch_inputs_tokenized = {
                    k: v.to(self.device) for k, v in batch_inputs_tokenized.items()
                }

                # Extract batch inputs
                batch_input_ids = batch_inputs_tokenized["input_ids"]
                batch_attention_mask = batch_inputs_tokenized.get(
                    "attention_mask", None
                )
    ...
    def generate_with_stop_condition(
        ...
        if infer_type not in {"dit", "llm_dit"}:
            error_msg = (
                f"invalid infer_type: {infer_type!r} (expected 'dit' or 'llm_dit')"
            )
            return {
        ...
        if is_batch:
            if seeds is None:
                ...
                seeds = list(seeds) + [
                    ...
                    for _ in range(actual_batch_size - len(seeds))
                ]
            else:
        ...
        if not has_all_metas and use_cot_metas:
            if is_batch:
                logger.info(
                    "Batch Phase 1: Generating CoT metadata (once for all items)..."
                )
            else:
            ...
            formatted_prompt = self.build_formatted_prompt(
                caption, lyrics, generation_phase="cot"
            )

            logger.info(
                f"generate_with_stop_condition: formatted_prompt={formatted_prompt}"
            )
            ...
            if is_batch:
                ...
                logger.info(
                    f"Phase 1 completed in {phase1_time:.2f}s. Generated metadata: {list(metadata.keys())}"
                )
        else:
            # Use user-provided metadata
            if is_batch:
                logger.info(
                    "Batch Phase 1: Using user-provided metadata (skipping generation)"
                )
            else:
                logger.info(
                    "Phase 1: Using user-provided metadata (skipping generation)"
                )
        ...
        if is_batch:
            logger.info(
                f"Batch Phase 2: Generating audio codes for {actual_batch_size} items..."
            )
        else:
        ...
        logger.info(
            f"generate_with_stop_condition: formatted_prompt_with_cot={formatted_prompt_with_cot}"
        )

        progress(
            0.5, f"Phase 2: Generating audio codes for {actual_batch_size} items..."
        )
        if is_batch:
            ...
            # Log results
            codes_counts = [
                len(codes.split("<|audio_code_")) - 1 if codes else 0
                for codes in audio_codes_list
            ]
            logger.info(
                f"Batch Phase 2 completed in {phase2_time:.2f}s. Generated codes: {codes_counts}"
            )
            ...
            codes_count = (
                len(audio_codes.split("<|audio_code_")) - 1 if audio_codes else 0
            )
            logger.info(
                f"Phase 2 completed in {phase2_time:.2f}s. Generated {codes_count} audio codes"
            )
    ...
    def build_formatted_prompt(
        ...
        if self.llm_tokenizer is None:
            raise ValueError(
                "LLM tokenizer is not initialized. Call initialize() first."
            )
    ...
    def understand_audio_from_codes(
        ...
        Args:
            ...
            min_p: Min-P sampling (None = disabled)
        ...
        logger.info(
            f"Understanding completed. Generated {len(metadata)} metadata fields"
        )
    ...
    def create_sample_from_query(
        ...
        if (
            ...
            and vocal_language.strip().lower() != "unknown"
        ):
    ...
    def format_sample_from_input(
        ...
        logger.info(
            f"Formatting sample from input: caption={caption[:50]}..., lyrics length={len(lyrics)}"
        )
        ...
        if user_metadata:
            ...
            if not constrained_metadata:
                ...
                logger.info(
                    f"Using user-provided metadata constraints: {constrained_metadata}"
                )
        ...
        output_text, status = self.generate_from_formatted_prompt(
            ...
            cfg={
                ...
                "skip_caption": False,  # Generate caption
                "skip_language": constrained_metadata.get("language") is not None
    ...
    def generate_from_formatted_prompt(
        ...
        skip_language = cfg.get(
            "skip_language", False
        )  # Skip language generation in CoT
        ...
        try:
            if self.llm_backend == "vllm":
                ...
                return (
                    output_text,
                    f"✅ Generated successfully (vllm) | length={len(output_text)}",
                )
                ...
                return (
                    output_text,
                    f"✅ Generated successfully (mlx) | length={len(output_text)}",
                )
            ...
            return (
                output_text,
                f"✅ Generated successfully (pt) | length={len(output_text)}",
            )
            ...
            logger.error(
                f"Error in generate_from_formatted_prompt: {type(e).__name__}: {e}\n{error_detail}"
            )
    ...
    def _generate_with_constrained_decoding(
        ...
        with torch.inference_mode():
            for step in tqdm(
                ...
                desc="LLM Constrained Decoding",
                unit="token",
                disable=self.disable_tqdm,
            ):
                # Forward pass
                outputs = self._forward_pass(
                    model, generated_ids, model_kwargs, past_key_values, use_cache
                )
                ...
                if constrained_processor is not None:
                    next_token_logits = constrained_processor(
                ...
                self._update_constrained_processor_state(
                    constrained_processor, next_tokens
                )

                # Check for EOS token
                should_stop = self._check_eos_token(
                    next_tokens, eos_token_id, pad_token_id
                )

                # Append token to sequence
                next_tokens_unsqueezed = next_tokens.unsqueeze(1)
                generated_ids = torch.cat(
                    [generated_ids, next_tokens_unsqueezed], dim=1
                )
                attn_mask = torch.cat(
                    [
                        attn_mask,
    ...
    def _generate_with_cfg_custom(
        ...
        device = self.device
        batch_size = (
            batch_input_ids.shape[0] // 2
        )  # Half are conditional, half are unconditional
        cond_start_idx = 0
        ...
        with torch.inference_mode():
            for step in tqdm(
                ...
                cond_logits = next_token_logits[
                    cond_start_idx : cond_start_idx + batch_size
                ]
                uncond_logits = next_token_logits[
                    uncond_start_idx : uncond_start_idx + batch_size
                ]
                ...
                cfg_logits = uncond_logits.float() + cfg_scale * (
                    cond_logits.float() - uncond_logits.float()
                )
                ...
                next_tokens_unsqueezed = next_tokens.unsqueeze(1)
                generated_ids = torch.cat(
                    [generated_ids, next_tokens_unsqueezed.repeat(2, 1)], dim=1
                )
        ...
        try:
            ...
            try:
                ...
                if not sample_key.startswith("model."):
                    logger.info(
                        "Adding 'model.' prefix to weight keys for MLX compatibility"
                    )
    ...
    def get_hf_model_for_scoring(self):
        ...
        if self.llm_backend == "pt":
            ...
            if self._hf_model_for_scoring is None:
                logger.info(
                    "Loading HuggingFace model for scoring (MLX backend, need PyTorch model)"
                )
                ...
                if model_path is None:
                    raise ValueError(
                        "MLX model path not stored. Cannot load HuggingFace model for scoring."
                    )
