...
class TrainingDecompositionContractTests(unittest.TestCase):
    ...
    def test_training_facade_imports_tab_helpers(self) -> None:
        ...
        self.assertIn(
            "acestep.ui.gradio.interfaces.training_lora_tab", imported_modules
        )
        self.assertIn(
    ...
    def test_training_ui_markers_preserved(self) -> None:
        ...
        expected_markers = {
            "training.py": ["🎵 Training for ACE-Step"],
            "training_dataset_tab_scan_settings.py": [
        ...
        for module_name, markers in expected_markers.items():
            ...
            for marker in markers:
                self.assertIn(
                    marker, source, f"Missing marker {marker!r} in {module_name}"
                )
