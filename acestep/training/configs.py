...
class VaeTrainingConfig:
    """Configuration for VAE decoder fine-tuning.

    Fine-tuning targets only the ``AutoencoderOobleck`` decoder so the encoder
    stays frozen. Training minimises a weighted combination of reconstruction
    loss (L1 + multi-scale STFT) directly on whole-file 48 kHz stereo
    waveforms.
    ...
    Attributes:
        ...
        pin_memory: Pin DataLoader tensors to page-locked memory.
        prefetch_factor: DataLoader prefetch factor per worker.
        persistent_workers: Keep workers alive between epochs.
        pin_memory_device: CUDA device string for pin_memory_device kwarg.
        log_every_n_steps: Optimizer steps between log/yield calls.
        val_split: Fraction of dataset held out for validation [0, 1).
        stft_loss_weight: Weight for multi-scale STFT loss vs L1 loss.
