...
class GenerationParams:
    ...
    Attributes:
        ...
        infer_method: Diffusion inference method. One of: "ode", "sde", "jkass_quality".
        ...
        lm_min_p: LLM min-p sampling (0 = disabled). Filters out tokens below min_p * max_prob.
...
def _update_metadata_from_lm(
    ...
    if not time_signature and metadata.get("timesignature"):
        time_signature_value = metadata.get(
            "timesignature", metadata.get("time_signature", "")
        )
...
def generate_music(
    ...
    try:
        ...
        user_provided_audio_codes = bool(
            params.audio_codes and str(params.audio_codes).strip()
        )
        ...
        if params.task_type == "cover":
            ...
            if no_src_audio and not user_provided_audio_codes:
                logger.warning(
                    "Cover task requested without source audio or audio codes. Falling back to text2music."
                )
                params.task_type = "text2music"
                if params.instruction == TASK_INSTRUCTIONS.get("cover"):
                    params.instruction = TASK_INSTRUCTIONS.get(
                        "text2music", params.instruction
                    )
        ...
        need_lm_for_cot = (
            params.use_cot_caption or params.use_cot_language or params.use_cot_metas
        )
        use_lm = (
        ...
        if params.task_type in skip_lm_tasks:
            logger.info(
                f"Skipping LM for task_type='{params.task_type}' - using DiT directly"
            )
        ...
        try:
            ...
            if (audio_duration is None or float(audio_duration) <= 0) and (
                params.src_audio or params.reference_audio
            ):
                ...
                try:
                    ...
                    if inferred and inferred > 0:
                        ...
                        logger.info(
                            f"[generate_music] Inferred duration from audio file: {inferred:.2f}s"
                        )
                except Exception as e:
                    logger.warning(
                        f"[generate_music] Failed to infer duration from audio file: {e}"
                    )
            ...
            if (
                ...
                and float(audio_duration) > max_duration
            ):
                logger.warning(
                    f"[generate_music] Duration {audio_duration}s exceeds GPU limit {max_duration}s. Clamping."
                )
            ...
            if config.batch_size is not None and config.batch_size > max_batch:
                logger.warning(
                    f"[generate_music] Batch size {config.batch_size} exceeds GPU limit {max_batch}. Clamping."
                )
            ...
            if (
                ...
                logger.warning(
                    "[generate_music] MPS with long duration detected; reducing batch size to 1 to avoid OOM."
                )
                config.batch_size = 1
        except Exception as e:
            logger.warning(
                f"[generate_music] Failed to clamp duration/batch to GPU limits: {e}"
            )
        ...
        if use_lm:
            ...
            max_inference_batch_size = (
                int(config.lm_batch_chunk_size)
                if config.lm_batch_chunk_size > 0
            ...
            for chunk_idx in range(num_chunks):
                ...
                chunk_end = min(
                    chunk_start + max_inference_batch_size, actual_batch_size
                )
                chunk_size = chunk_end - chunk_start
                chunk_seeds = (
                    actual_seed_list[chunk_start:chunk_end]
                    if chunk_start < len(actual_seed_list)
                    else None
                )
                ...
                if lm_chunk_time_costs:
                    ...
                    time_str = ", ".join(
                        [f"{k}: {v:.2f}s" for k, v in lm_chunk_time_costs.items()]
                    )
            ...
            if infer_type == "llm_dit":
                ...
                if actual_batch_size > 1:
                    ...
                    audio_code_string_to_use = (
                        all_audio_codes_list[0] if all_audio_codes_list else ""
                    )
            else:
            ...
            if params.use_cot_caption:
                dit_input_caption = lm_generated_metadata.get(
                    "caption", dit_input_caption
                )
        ...
        for idx, dit_audio in enumerate(dit_audios):
            ...
            if lm_generated_audio_codes_list and idx < len(
                lm_generated_audio_codes_list
            ):
            ...
            audio_code_str = (
                ...
                if (
                    lm_generated_audio_codes_list
                    and idx < len(lm_generated_audio_codes_list)
                )
                else audio_code_string_to_use
            )
            if isinstance(audio_code_str, list):
                audio_code_str = (
                    audio_code_str[idx] if idx < len(audio_code_str) else ""
                )
            ...
            if audio_tensor is not None:
                silent_check, rms_val, peak_val = is_audio_silent(
                    audio_tensor, channels_first=True
                )
                if silent_check:
                    logger.warning(
...
def understand_music(
    ...
    Args:
        ...
        min_p: Min-P sampling (None or 0 = disabled)
